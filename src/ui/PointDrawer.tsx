...
function EsBadge({ show }: { show: boolean }) {
  if (!show) return null;
  return (
    <span className="ml-1 rounded bg-white/10 px-1 py-px text-[9px] tracking-wider text-zinc-400 uppercase">ES</span>
  );
}
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-4">
        ...
        <div className="mb-4 flex flex-wrap gap-1.5 text-[11px]">
          ...
          {point.element ? (
            <span className="rounded-full bg-white/10 px-2 py-1 text-zinc-300">{t(locale, point.element)}</span>
          ) : null}
          {point.polaridad ? (
            <span className="rounded-full bg-white/10 px-2 py-1 text-zinc-300">{t(locale, point.polaridad)}</span>
          ) : null}
          <span className="rounded-full bg-white/10 px-2 py-1 text-zinc-300">
            {point.laterality}
        ...
        <section className="mb-4">
          <h3 className="mb-1 text-[10px] tracking-[0.18em] text-zinc-500 uppercase">
            {t(locale, "location")}
            <EsBadge show={esOnly} />
          </h3>
        ...
        {point.precautions.length > 0 ? (
          <section className="mb-4 rounded-xl border border-rose-300/20 bg-rose-950/30 p-3">
            <h3 className="mb-1 text-[10px] tracking-[0.18em] text-rose-200 uppercase">
